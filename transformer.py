...
class AttentionHead(nn.Module):
    ...
    def __init__(self, n_embd, head_size, block_size, dropout_prob):
        """
    ...
    def forward(self, x, is_decoder=False):  # Compute single head attention
        ...
        attention_head_weights = query @ key.transpose(-2, -1) * key.shape[-1] ** -0.5
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_embd, n_head, head_size, block_size, dropout_prob):
        ...
        self.attention_heads = nn.ModuleList([AttentionHead(
            ...
            dropout_prob=dropout_prob) for _ in range(n_head)])
...
class Encoder(nn.Module):
    ...
    def forward(self, idx):
        ...
        position_embedding = self.position_embedding_table(torch.arange(end=num_tokens, device=device))
...
class Decoder(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        num_batches, num_tokens = idx.shape
