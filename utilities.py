...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        word_ids = self.tokenizer.encode(sentence)

        # Prepare the padded input for the model
        padded_sentence = word_ids[:block_size] + [0] * (block_size - len(word_ids))
        ...
        _, attn_maps = self.model(
            input_tensor)  # Ignore the output of the model, and only get the attention maps; make sure your encoder
